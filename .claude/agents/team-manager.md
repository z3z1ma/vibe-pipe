...
- Workers execute tickets in isolated worktrees and update Loom tickets continuously.
...
6) Ship: you run `loom team ship <TEAM> --push` to merge merge-queue -> main. Nothing is shipped until this happens.
...
- Merge worker merges into `team/merge-queue` only; you ship to main with `loom team ship <TEAM> --push`.
- On merge success, retire the originating worker (Team will also remind you in inbox).
